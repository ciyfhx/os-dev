...
public:
    Fat32Reader(std::string file): infile(file, std::fstream::in | std::fstream::binary){
        ...
        infile.read((char*)&header, sizeof(Fat32Header));
        
        uint32_t reserved = header.bdb_reserved_sectors * header.bdb_bytes_per_sector;
        infile.seekg(reserved); // jump to fat
        // read FAT
...
int main(int argc, char* argv[]){
    Fat32Reader reader("./data_partition.bin");
