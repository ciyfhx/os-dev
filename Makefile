...
CPPFLAGS = -g -fvar-tracking -std=c++26 -B/usr/local/x86_64elfgcc/bin/ -fpermissive -fno-exceptions -fno-rtti -fno-PIE 
LDFLAGS = -melf_x86_64 
...
${BIN_DIR}/kernel.bin: ${BIN_DIR}/kernel/kernel_entry.o ${KERNEL_ASM_OBJS_PATH} ${KERNEL_CPP_OBJS_PATH}
	${LD} ${LDFLAGS} -o $@ -T ./kernel/linker.ld $^ --oformat binary 

${BIN_DIR}/kernel/%.o: kernel/%.cpp ${HEADERS}
	${CXX} ${CPPFLAGS} -mcmodel=large -m64 -ffreestanding  -c $< -o $@ 
...
${BIN_DIR}/kernel.elf: ${BIN_DIR}/kernel/kernel_entry.o ${KERNEL_ASM_OBJS_PATH} ${KERNEL_CPP_OBJS_PATH}
	${LD} ${LDFLAGS} -o $@ -T ./kernel/linker.ld $^ 
